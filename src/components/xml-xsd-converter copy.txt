...
const XMLXSDConverter: React.FC = () => {
  ...
  // 2. Enhanced debugging in generateTablePreview
  const generateTablePreview = (xmlContent: string): string => {
    ...
    const processChildToTableRow = (node: Element, level: number): string => {
      ...
      if (node.children.length === 0) {
        ...
        // ENHANCED: Better debugging and matching
        console.log(`Processing element: ${elementName}`);
        console.log(`Available schema keys:`, Object.keys(schemaInfo));
        console.log(`Schema info for ${elementName}:`, schemaInfo[elementName]);
        ...
        // ENHANCED: Try alternative matching strategies
        let matchedSchema = elementSchema;
        if (!hasEnumValues) {
          // Try case-insensitive matching
          const lowerElementName = elementName.toLowerCase();
          const matchingKey = Object.keys(schemaInfo).find(
            (key) => key.toLowerCase() === lowerElementName
          );
          if (matchingKey) {
            matchedSchema = schemaInfo[matchingKey];
            console.log(
              `Found case-insensitive match: ${matchingKey} for ${elementName}`
            );
          }

          // Try partial matching (remove prefixes/suffixes)
          if (!matchedSchema?.enumValues) {
            const partialMatch = Object.keys(schemaInfo).find(
              (key) => key.includes(elementName) || elementName.includes(key)
            );
            if (partialMatch) {
              matchedSchema = schemaInfo[partialMatch];
              console.log(
                `Found partial match: ${partialMatch} for ${elementName}`
              );
            }
          }
        }

        const hasMatchedEnumValues =
          matchedSchema?.enumValues && matchedSchema.enumValues.length > 0;

        result += `<tr class="data-row">
        <td class="field-name">${formatTagName(node.tagName)}</td>
        <td class="field-value">`;

        if (hasMatchedEnumValues) {
          console.log(
            `Creating dropdown for ${elementName} with values:`,
            matchedSchema.enumValues
          );
          ...
          onchange="window.updateXMLValue('${elementName}', this.value)"
        >`;

          matchedSchema.enumValues!.forEach((enumValue) => {
  ...
  const _parseXSDSchema = (xsdContent: string) => {
    try {
      const parser = new DOMParser();
      const xsdDoc = parser.parseFromString(xsdContent, 'application/xml');
